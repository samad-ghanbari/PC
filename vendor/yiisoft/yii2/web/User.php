...
class User extends Component
{
    ...
    public function getAreaList($project_id, $keyValue=false)
    {
        $user_id = Yii::$app->user->getId();
        $up_id = \app\models\UserProjects::find()->select('id')->where(['user_id'=>$user_id, 'project_id'=>$project_id])->scalar();
        if(empty($up_id)) return [];
        $upMeta = \app\models\UserProjectMeta::find()->select("value")->where(['user_project_id'=>$up_id,'key'=>"area"])->asArray()->all(); // number | *
        $array = [];
        foreach ($upMeta as $m)
            $array[] = $m['value'];
        if(in_array("*", $array))
    ...
    public function getCenterList($project_id, $withEmpty=false)
    {
        // withEmpty includes -1=>"" in each area
        // [2=>[1=>"x", ....] , 3=>[] ] >> [AREA=>[id=>name, ...]  ... ]
        $res = [];
        $array_int = [];
        $array_area = [];
        $star = false;
        ...
        $user_id = Yii::$app->user->getId();
        $areas = $this->getAreaList($project_id);// [2,3]

        $up_id = \app\models\UserProjects::find()->select('id')->where(['user_id'=>$user_id, 'project_id'=>$project_id])->scalar();
        if(empty($up_id)) return [];
        $upMeta = \app\models\UserProjectMeta::find()->select("value")->where(['user_project_id'=>$up_id,'key'=>"center_id"])->asArray()->all();
        if($withEmpty)
        {
            foreach ($areas as $a)
                $res[$a][-1] = "";
        ...
        // 12 >>> center_id: 12
        // 2:*  all area2 centers
        // *    all centers in defined areas

        foreach ($exchanges as $e)
        {
            $val = trim($e);
            if(is_numeric($val))
                $array_int[] = intval($val);    // [ centerIds ]
            else if(str_contains($val, ':*'))
                $array_area[] = $val;           // [ areas ]
            else if(str_contains($val, '*'))
            {
                $star = true;
                break;
        ...
        if($star == true)
        {
            // all centers but in accessable areas
            $q = \app\models\BaseSitex::find()->select("id, area, name")->where(['area'=>$areas, 'type'=>"مرکز"])->asArray()->all();
            foreach($q as $rec)
                $res[$rec['area']][$rec['id']] = $rec['name'];
        }
        else
        {
            // array_int [2, 3, 4...]
            //get id area name
            $q = \app\models\BaseSitex::find()->select("id, area, name")->where(['id'=>$array_int, 'type'=>"مرکز"])->asArray()->all();
            foreach($q as $rec)
                $res[$rec['area']][$rec['id']] = $rec['name'];
            ...
            //get id area name
            $q = \app\models\BaseSitex::find()->select("id, area, name")->where(['area'=>$AREA_LIST, 'type'=>"مرکز"])->asArray()->all();
            foreach($q as $rec)
                $res[$rec['area']][$rec['id']] = $rec['name'];
        }
    ...
    public function getCentersAccessCondition($project_id)
    {
        // [ ['area' => 2] , ['in','id',[1,2,3,4, ], [], []   ] to use ->orWhere()
        ...
        $up_id = \app\models\UserProjects::find()->select('id')->where(['user_id'=>$user_id, 'project_id'=>$project_id])->scalar();
        if(empty($up_id)) return ['area'=>-1]; // false condition
        $upMeta = \app\models\UserProjectMeta::find()->select("value")->where(['user_project_id'=>$up_id,'key'=>"center_id"])->asArray()->all();
        $exchanges = [];
        foreach ($upMeta as $m)
            $exchanges[] = $m['value'];
        ...
        {
            // array_int [2, 3, 4...]
            $res[] = ['area'=>$areas, 'sitex_id'=>$array_int];

//            $q = \app\models\BaseSitex::find()->select("id")->where(['id'=>$array_int, 'area'=>$areas])->asArray()->all();
//            $array = [];
//            foreach ($q as $rec)
//                $array[] = $rec['id'];
//            if(!empty($array))
//                $res[] = ['id'=>$array];
        ...
        if(empty($res)) $res[] = ['area'=>-1];
        return $res;
    }

    public function canAccessArea($area, $project_id)
    {
        // check user can access are x in project Y
        // if user project area is * , so user can access

        $user_id = Yii::$app->user->getId();
        $up_id = \app\models\UserProjects::find()->select('id')->where(['user_id'=>$user_id, 'project_id'=>$project_id])->scalar();
        if(empty($up_id)) return false;

        $upMeta = \app\models\UserProjectMeta::find()->select("value")->where(['user_project_id'=>$up_id,'key'=>"area"])->asArray()->all();
        $array = [];
        foreach ($upMeta as $m)
            $array[] = $m['value'];
        if(in_array("*", $array)) return true;
        if(in_array(strval($area) , $array)) return true; else return false;
    }

    public function canAccessCenter($center_id, $project_id)
    {
        // the exchange area is accessable
        // user_project_meta has this center_id
        // get array of center_id of up_meta >> [number or 2:*]

        $user_id = Yii::$app->user->getId();
        $up_id = \app\models\UserProjects::find()->select('id')->where(['user_id'=>$user_id, 'project_id'=>$project_id])->scalar();
        if(empty($up_id)) return false;

        $area = \app\models\BaseSitex::find()->select("area")->where(['id'=>$center_id])->scalar();
        if($this->canAccessArea($area, $project_id))
        {
            $upMeta = \app\models\UserProjectMeta::find()->select("value")->where(['user_project_id'=>$up_id,'key'=>"center_id"])->asArray()->all();
            $exchanges = [];
            foreach ($upMeta as $m)
            ...
            foreach ($exchanges as $e)
            {
                $val = $e;
                if(is_numeric($val))
                    $array_int[] = intval($val);
                else if(str_contains($val, '*'))
                    $array_str[] = $val;
            }

            if(in_array($center_id, $array_int)) return true;
    ...
    { // project defined for this user
        $user_id = Yii::$app->user->getId();
        $up_id = \app\models\UserProjects::find()->select('id')->where(['user_id'=>$user_id, 'project_id'=>$project_id])->scalar();
        if((!empty($up_id)) && ($up_id > 0) ) return true;

        Yii::$app->session->setFlash("error", "شما دسترسی لازم جهت مشاهده این پروژه را ندارید.");
        return false;
    }

    public function canViewProject($project_id)
    {
        //user can view project
        if($this->canAccessProject($project_id))
        {
            $pv = \app\models\BaseProjects::findOne($project_id)->visible;
            if($pv)
            {
                $upv = \app\models\UserProjects::find()->where(['user_id'=>Yii::$app->user->getId(), 'project_id'=>$project_id])->one()->visible;
                if($upv) return true;
                else
                {
                    Yii::$app->session->setFlash("error", "شما دسترسی لازم جهت مشاهده این پروژه را ندارید.");
            ...
            {
                Yii::$app->session->setFlash("warning", "امکان مشاهده پروژه میسر نمی‌باشد.");
                return false;
            }
        }
        else
    ...
    {
        $access = ['Access Admin'=>false, 'Access Statistics'=>false, 'Create Record'=>false, 'Import From Excel'=>false, 'Remove Record'=>false, 'Update Record'=>false, 'Update Sitex'=>false];

        if($this->can("Access Admin")) $access['Access Admin'] = true;
        if($this->can("Access Statistics")) $access['Access Statistics'] = true;
        if($this->can("Create Record")) $access['Create Record'] = true;
        if($this->can("Import From Excel")) $access['Import From Excel'] = true;
        if($this->can("Remove Record")) $access['Remove Record'] = true;
        if($this->can("Update Record")) $access['Update Record'] = true;
        if($this->can("Update Sitex")) $access['Update Sitex'] = true;
